...
ggplot(data = Western_Europe) + 
  ...
  ggtitle("Long-term Trend Homicides Western Europe")
```
...
ggplot(data = Western_Europe) + 
  #... YOUR CODE
geom_line(mapping=aes(x=Year,
                        y=homicides_per_100k, colour = Country))+
...
```{r eval = TRUE}
# YOUR CODE HERE:
Denmark<-read_csv("kings1.csv",na=c("NULL",""))
Denmark %>% 
  mutate(Duration = End_year - Start_year) %>% 
  mutate(Midyear = End_year - (End_year - Start_year)/2 ) %>% 
  ggplot(mapping = aes(x = Midyear,y= Duration))+
  geom_point()+
  geom_smooth()+
  theme_bw()+
  xlab("Midyear regency")+
  ylab("Duration of reign")+
  ggtitle("Duration of Danish Kings and Queens")

```

In the danish data the duration of the kings and queens become longer with the years. If we compare it to the Western European data over homicides, we can see that the ammount of homicides also fall with the years, with a drastic change around 1400-1500. 
...
If we look at the graphs over homicide rates in Western Europe, the trend throughout the different countries show a downward trend in homicides. The y-axis for the number of homicides are counted per 100.000 people, so it might be a larger number than first anticipated, but overall, we have become more civilized in terms of homicides. 

Article - falling homicide rates: https://www.goodnewsnetwork.org/global-homicide-rates-continue-to-fall-2020/ 
